...
def gerar_recomendacoes_opcao_c(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for codigo in sorted(codigo_avaliador_stats.keys(), 
        ...
        intensidade = "🔴 CRÍTICO" if percentual_consenso >= 70 else "🟠 IMPORTANTE" if percentual_consenso >= 50 else "🟡 ATENÇÃO"
...
def gerar_pontos_positivos(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for result in results:
        for desconto in result['descontos_detalhados']:
            codigo = desconto['codigo']
            codigo_frequencia[codigo] = codigo_frequencia.get(codigo, 0) + 1
    ...
    for categoria in CATEGORIES:
        erros = categoria_erros.get(categoria, 0)
    ...
    return {
        ...
        'forca_relativa': {
            'codigos': sorted(codigos_forca_relativa.keys(), key=lambda x: codigos_forca_relativa[x]['percentual']),
            'detalhes': codigos_forca_relativa
        },
        'categoria_desempenho': categoria_desempenho,
        'alunos_destaque': [{'nome': r['nome'], 'nota_final': r['nota_final']} for r in alunos_destaque]
    }

# ============================================================================
# GERAÇÃO DE RELATÓRIO MASTER
# ============================================================================
def gerar_relatorio_master(results, suffix, recomendacoes, pontos_positivos):
    """Gera relatório master com encoding utf-8-sig para Telegram."""
    ...
    # O SEGREDO: utf-8-sig adiciona o BOM que resolve os caracteres quebrados no Telegram
...
def run():
    ...
    if not exame_files:
        logger.warning("Nenhum arquivo de exame encontrado.")
        return
    ...
    for filepath in exame_files:
        ...
        try:
            ...
            # SRE: shutil.move é atômico e seguro entre volumes
            shutil.move(str(filepath), str(PROCESSED_DIR / filepath.name))
            logger.info(f"Arquivo movido para processados.")
