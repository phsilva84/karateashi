...
from core.calculator import compute_student_result, analisar_pedagogico

logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')
logger = logging.getLogger(__name__)

def gerar_txt(results, suffix, recomendações, elogios):
    master_file = OUTPUT_DIR / f"relatorio_master_dojo_{suffix}.txt"
    media = sum(r['nota_final'] for r in results) / len(results) if results else 0.0
    
    with open(master_file, 'w', encoding='utf-8-sig') as f:
        f.write(f"=== RELATÓRIO MASTER DO DOJO - {suffix.upper()} ===\n")
        f.write(f"Média Geral: {media:.2f}\n\n")
...
def run():
    ...
    for filepath in sorted(DATA_DIR.glob('exame-*.txt')):
        ...
        # IDEMPOTÊNCIA: Se o relatório já existe no output, não reprocessa
        if master_file.exists():
            logger.info(f"SKIP: {suffix} já processado. Movendo arquivo.")
